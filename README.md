...
| `PARTNERS` | Кнопка меню «Наши партнёры» — список компаний: записи `Название \| https://url` через `;` (переносы строк тоже работают). Пусто → кнопка скрыта | Нет |
| `PARTNERS_URL` | Legacy: одна ссылка, используется как fallback если `PARTNERS` пуст | Нет |
