...
| `PARTNERS` | Кнопка меню «Наши партнёры»: по одному `Название \| https://url` на строку (или через `;`). Пусто → кнопка скрыта | Нет |
| `PARTNERS_URL` | Legacy: один партнёр, используется как fallback если `PARTNERS` пуст | Нет |
