...
и русскоязычная аудитория. Переводы есть на 7 языков (`az, ru, en, tr, kz,
uz, ar`), но `uz` и `ar` временно убраны из выбора языка — в боте доступны
`az, ru, en, tr, kz`.
...
Страница `/broadcast` умеет:

- **HTML-разметку Telegram** — жирный, курсив, зачёркнутый, моноширинный,
  спойлер и гиперссылки (`<a href="https://…">текст</a>`);
- **inline-кнопки** со ссылками под сообщением (до 8 штук);
- **отложенную отправку** — время вводится в МСК, задание хранится в БД и
  переживает редеплой воркера; до момента отправки её можно отменить.

Разметка и ссылки проверяются до постановки в очередь: битый тег Telegram
отклоняет целиком, поэтому такая рассылка не должна доходить до отправки.
...
| `PARTNERS` | **Только bootstrap.** Импортируется один раз в пустую таблицу `partners`, дальше игнорируется — партнёрами управляют в дашборде (`/partners`). Формат: `Название \| https://url` через `;` | Нет |
| `PARTNERS_URL` | Legacy: одна ссылка, тот же bootstrap-only режим, fallback если `PARTNERS` пуст | Нет |
| `PARTNER_REDIRECT_BASE` | Публичный URL дашборда для подсчёта кликов по партнёрам. Пусто → кнопки ведут напрямую, кликов не видно | Нет |
| `PARTNER_CPA` | Стоимость одного партнёрского клика. Пусто → дашборд показывает воронку без денег; задано → считает прогноз дохода на пользователя (не фактическая выручка) | Нет |
